#' @title Classifica códigos da CID-10 em Capítulos
#' @aliases cid10cap
#'
#' @description Tomando um vetor com códigos da 10ª Revisão da Classificação Internacional de Doenças (CID-10), acrescenta uma variável com os capítulos da CID-10 correspondentes.
...
cid10cap <- function(cid, droplevels = FALSE) {
  ...
  capcid <- dplyr::case_when(cids <= "B99" ~ 1,
                             cids >= "C00" & cids <= "D48" ~ 2,
                             cids >= "D50" & cids <= "D89" ~ 3,
                             cids >= "E00" & cids <= "E90" ~ 4,
                             cids >= "F00" & cids <= "F99" ~ 5,
