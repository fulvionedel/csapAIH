...
#' @param nomes Nomes dos grupos a serem apresentados no gráfico. TRUE (padrão), usa os nomes da lista usada ("MS" ou "Alfradique"); FALSE usa o código dos grupos ("g01", ..., "g19"/"g20"). Permite a atribuição de nomes de uma lista, com \code{nomes = <meus.nomes>}.
...
#' desenhaCSAPdev2(df, titulo = "auto", onde = "Rio Grande do Sul")
#'
#' # Sem titulo e sem ordenacacao por frequencia:
#' desenhaCSAPdev2(df, ordenar = FALSE)
#' #
#' # Com a lista de Alfradique et al.:
#' desenhaCSAPdev2(csapAIH(aih100, "Alfradique"),
#'             lista = "Alfradique",
#'             titulo = "auto",
#'             onde = "Rio Grande do Sul")
#' desenhaCSAPdev2(csapAIH(aih100, "Alfradique"), lista = "Alfradique", lang = "es")
#'
#' # Cores
#' #-------
#' # Sem cores nas barras
#' desenhaCSAPdev2(df, colorir = FALSE)
#'
#' # Com as barras em tons de cinza
#' # desenhaCSAPdev2(df, colorir = "cinza")
#'
#' # Com as barras em outra cor
#' desenhaCSAPdev2(df, colorir = "yellow")
#' #
...
#' # ---------------------------------------
#' fator <- df$grupo
#' desenhaCSAPdev2(fator)
#' \dontrun{
#' carater <- as.character(fator)
#' desenhaCSAPdev2(carater, limsup = 4.4)
#'
#' # Se \code{titulo = "auto"}, o argumento \code{quando} eh obrigatorio:
#' \dontrun{
#'  desenhaCSAPdev2(carater, titulo = "auto", onde = 'RS') # resulta em erro
#'  }
#'  desenhaCSAPdev2(carater, titulo = "auto", onde = "RS", quando = "jan/2012")
#'  desenhaCSAPdev2(carater, titulo = "Título manual")
#' }
#'
...
#' desenhaCSAPdev2(tabela, jaetabela = TRUE)
#' # \dontrun{
#' # Resulta em erro, faltou o argumento 'quando'
#' desenhaCSAPdev2(tabela, jaetabela = TRUE, titulo = "auto", onde = 'RS')
#' # }
#' desenhaCSAPdev2(tabela, jaetabela = TRUE,
#'             titulo = "auto", onde = "RS", quando = "jan/2012")
#' desenhaCSAPdev2(tabela, jaetabela = TRUE, titulo = "Título manual")
#' }
#'
#' #  Graficos com as funcoes basicas
#' # =================================
#' \dontrun{
#' desenhaCSAPdev2(df, tipo.graf = "base", titulo = "dados = um banco")
#' desenhaCSAPdev2(df$grupo, tipo.graf = "base", titulo = "dados = um fator")
#' # desenhaCSAPdev2(tabela, jaetabela = TRUE, tipo.graf = "base", titulo = "dados = uma tabela")
#' }
#'
...
#' @export
#'
desenhaCSAPdev2 <- function(dados, tipo = 1, lista = "MS", lang = "pt.ca", nomes = TRUE, tipo.graf = "ggplot", valores = "porcento", ordenar = TRUE, colorir = TRUE, porcentagens = TRUE, val.dig = 0, titulo = NULL, onde, quando = NULL, t.hjust = 1, t.size = 12, x.size = 10, y.size = 11, val.size = 2.5, limsup = NULL, ...){
...
# ----------
  tab.fator <- function(dados, ...) {
    ...
    grupos <- table(grupos)
    tabela <- data.frame(cbind(Grupo = names(grupos),
                               Casos = grupos,
                               perc  = prop.table(grupos)*100))
    if(ordenar == TRUE) {
      tabela <- tabela[ order(tabela[, 'Casos'], decreasing = FALSE), ]
      # tabela <- left_join(tabela, nomes, by = join_by(Grupo == cod))
    }
    tabela
  }

  if(tipo == 2 | tipo == "fator") {
    if(is.character(dados)) { dados <- as.factor(dados) }
...
# Para tabela
# -----------
  if(tipo == 3 | tipo == "tabela") {
    tab.tabela <- function(...) {
      if("tabCSAP" %in% class(dados)) {
        tabela[1:(nrow(tabela)-3), ]
...
#
  ...
  if(tipo.graf == 'base' | "ggplot2" %in% rownames(installed.packages()) == FALSE) {
    ...
    if ( is.factor(dados) ) {
      ...
      grafico
      # return(grafico)
    } else if (tipo == 3 | tipo == "tabela") {
      grafico <- ggfactab(tab.tabela(dados))
      } else if(tipo == 1 | tipo == "banco") {
      # ----- Com o banco --------------------------------------------
      # Comandos exclusivos para o gráfico usando todo o banco de dados
        ...
        # if(ordenar == TRUE) { # Ordem decrescente de frequência
          protografico <- ggplot2::ggplot(df,
                            ggplot2::aes(x = reorder(grupo, grupo, function(x) length(x)), group=1))
        # } else
        if (ordenar == FALSE) {
          protografico <- ggplot2::ggplot(df, ggplot2::aes(x = grupo, group = 1))
        }
        #
        if(valores == "porcento") {
 ...
 # Colorir as barras
        if (colorir == TRUE) { # As cores das barras
          ...
          if(is.data.frame(dados) & tipo != 3) {
            grafico <- grafico +
              ggplot2::geom_bar(fill = cores)
          } else if(is.factor(dados) | tipo == 3) {
            grafico <- grafico +
            ggplot2::geom_col(fill = cores)
          }
        } else if (colorir != "cinza" & colorir != TRUE &  colorir != FALSE) {
            grafico <- protografico +
              ggplot2::aes(fill = cores, col = "black") +
              ggplot2::scale_fill_identity(aesthetics = c("fill", "colour")) }
        grafico <- grafico + ggplot2::theme(legend.position="none")
    ...
    if( !is.null(limsup) ) {
      if(valores == "porcento") {
        grafico <- grafico +
              ggplot2::scale_y_continuous(labels = function (x) paste(floor(x*100),"%"), limits = c(0, limsup))
      } else
        grafico <- # suppressMessages(print(
            grafico +
