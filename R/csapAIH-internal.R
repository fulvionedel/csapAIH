...
#'    \item \code{v0.0.4}
#'
#'    A função \code{listaBR} foi renomeada para \code{listaBRMS} e foi criada a função \code{listaBRAlfradique} para a classificação das ICSAP em 20 grupos de causa, conforme publicado em \href{https://doi.org/10.1590/S0102-311X2009000600016}{Alfradique et al. (2009)} -- ambas são funções internas invocadas pela função \code{\link{csapAIH}}. Incluído o argumento \code{lista} na função \code{\link{nomesgruposCSAP}} para adequar-se à lista com 20 grupos. O argumento \code{tipo} da função \code{\link{nomesgruposCSAP}} (e utilizado em \code{\link{desenhaCSAP}} e \code{\link{tabCSAP}}) foi renomeado para \code{lang}. Criadas as funções \code{\link{fxetar_quinq}}, para transformar um vetor numérico com a idade em faixas etárias quinquenais, e \code{\link{fxetar3g}}, para transformar um vetor numérico com a idade ou categórico com faixas etárias quinquenais em três grandes grupos etários: 0 a 14, 15 a 59 e 60 e + anos de idade. Criada a função \code{\link{cid10cap}}, para classificar códigos da CID-10 segundo seus capítulos. Criadas as funções \code{\link{ler_popbr}}, para leitura dos dados de população de 2008 a 2012 do DATASUS, armazenados no computador ou importados no site FTP (do DATASUS) e \code{\link{popbr2000_2021}}, com estimativas populacionais para os municípios brasileiros de 2000 a 2021. Incluído um banco de dados com a população dos municípios brasileiros por sexo e faixa etária, contada no Censo 2010 (\code{\link{POPBR10}}), e outro com uma amostra de mil registros da \emph{Encuesta de Egresos Hospitalarios} do Equador, ano 2020 (\code{\link{eeh20}}). Criada a função \code{\link{fetchcsap}}, uma forma abreviada da função \code{fetchdatasus} do pacote \code{microdatasus}, para o download dos arquivos da AIH e classificação das CSAP. Com o retorno à disponibilização dos arquivos de estimativa populacional pelo DATASUS, a função \code{\link{ler_popbr}} agora lê os arquivos de 1980 a 2024.
#'
#'    \item \code{v0.0.3.3}
#'
