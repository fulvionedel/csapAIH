...
#'    \item \code{v0.0.3.4}
#'
#'    A função \code{listaBR} foi renomeada para \code{listaBRMS} e foi criada a função \code{listaBRAlfradique} para a classificação das ICSAP em 20 grupos de causa, conforme publicado em \href{https://doi.org/10.1590/S0102-311X2009000600016}{Alfradique et al. (2009)} -- ambas são funções internas invocadas por \code{\link{csapAIH}}. Incluído o argumento \code{lista} na função \code{\link{nomesgruposCSAP}} para adequar-se à lista com 20 grupos. O argumento \code{tipo} da função \code{\link{nomesgruposCSAP}} (e utilizado em \code{\link{desenhaCSAP}} e \code{\link{tabCSAP}}) foi renomeado para \code{lang}. Criada a função \code{\link{fxetar_quinq}}, para transformar um vetor numérico com a idade em faixas etárias quinquenais. Incluída a função \code{\link{ler_popbr}}, para leitura dos arquivos de população de 2008 a 2012 no site FTP do DATASUS, e um banco de dados com a população dos municípios brasileiros por sexo e faixa etária, contada no Censo 2010.
#'
#'    \item \code{v0.0.3.3}
#'
