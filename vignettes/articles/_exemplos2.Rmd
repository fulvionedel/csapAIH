...
### Arquivos no servidor FTP do DATASUS 
...
Mude o argumento `sihsus` para `FALSE` e indique no argumento `cid` qual variável contém os códigos diagnósticos. As variáveis `csap` e `grupo` (se se mantiver o argumento `grupos`como `TRUE`) são acrescentadas ao final do banco de dados alvo da função.
...
A variável `cau_cie10` (posição `r which(names(eeh20) == 'cau_cie10')`) tem o código do diagnóstico de internação. A função csapAIH acrescenta à base duas (se `grupos = TRUE`) variáveis, `csap` e `grupo`, dispostas nas últimas colunas (posição `r paste(which(names(eeh20) == 'csap'), "e", which(names(eeh20) == 'grupo'), "respectivamente.")`)
